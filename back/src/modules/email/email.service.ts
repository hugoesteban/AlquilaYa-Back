...
export class EmailService {
  ...
  async sendEmailRegisterSuccessfully(email: string, name: string) {
    ...
    sgMail
      ...
      .catch((err) => {
        console.log('error al mandar el mail', err.message);
      });
  }

  async sendEmailCreatePropertySuccessfully(
    email: string,
    name: string,
    propertyId: string,
  ) {
  ...
  async sendEmailBookSuccesfully(
    email: string,
    bookingId?: string,
    paymentId?: string,
    propertyId?: string,
  ...
  async sendEmailForgotPassword(email: string) {
    const emailDb: Email =
      await this.emailRepository.createEmailRepository(email);
    sgMail.setApiKey(process.env.SENDGRID_API_KEY);
    sgMail
      .send({
        to: `${email}`,
